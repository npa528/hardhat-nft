    ...
    : describe("DynamicSvgNft", function () {
          ...
          describe("constructor", () => {
              it("sets starting values correctly", async function () {
                  assert.equal(await dynamicSvgNft.getLowSvg(), lowSVGImageuri)
                  assert.equal(await dynamicSvgNft.getHighSvg(), highSVGimageUri)

                  assert.equal(await dynamicSvgNft.name(), "Dynamic SVG NFT")
                  assert.equal(await dynamicSvgNft.symbol(), "DNS")

                  const tokenCounter = await dynamicSvgNft.getTokenCounter()
                  assert.equal(tokenCounter.toString(), "0")

                  assert.equal(await dynamicSvgNft.getPriceFeed(), ethUsdAggregator.address)
              })
          })
          ...
          describe("mintNft", () => {
              it("emits an event and creates the NFT", async function () {
                  ...
                  await expect(dynamicSvgNft.mintNft(highValue))
                      ...
                      .withArgs(tokenCounter, highValue)
